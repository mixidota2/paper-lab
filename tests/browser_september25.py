...
from test_september25_labs import SLUGS
ROOT = Path(__file__).resolve().parents[1]

def main():
    errors, states = [], 0
    shots = Path('/tmp/lab-sep25'); shots.mkdir(exist_ok=True)
            ...
            for slug in SLUGS:
                ...
                for w in page.locator('[data-b21]').all():
                    ...
                    for i, frame in enumerate(frames):
                        w.locator('select').select_option(index=i)
                        assert w.locator('[data-output]').inner_html() == page.evaluate('(s)=>{const d=document.createElement("div");d.innerHTML=s;return d.innerHTML}', frame), (slug, width, i, w.locator('[data-output]').inner_html(), frame)
                        states += 1
